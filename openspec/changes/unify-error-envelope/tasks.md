...
- [x] 4.2 Extend `tests/IceCreamTruck.WebApi.Tests/` with JSON body-shape assertions that verify the full pipeline end-to-end: for at least one non-success scenario per mapper (controller action + minimal endpoint), assert the response body contains the `type`, `title`, `status`, `detail`, and `messages` fields (D9)
- [x] 4.3 Create the five `docs/errors/*.md` files: `bad-request.md` (400), `forbidden.md` (403), `not-found.md` (404), `conflict.md` (409), `internal-server-error.md` (500); each file must contain at minimum the HTTP status code, the `type` URI it documents, a human-readable description, and an example JSON body
- [x] 4.4 Update `README.md`: remove the `ResultFilter` phantom section/reference; update any HTTP-mapping code examples to reflect the `application/problem+json` contract; correct any outdated status→HTTP code tables (D7)
- [x] 4.5 Add a `## [2.0.0-rc3]` breaking-change section to `CHANGELOG.md` describing the unified error envelope, removal of `ForbidResult`/`Results.Forbid()`, and the new `ProblemResponse`/`ErrorMessage` types
- [x] 4.6 Bump `<Version>` in `src/Directory.Build.props` from `2.0.0-rc2` to `2.0.0-rc3` in the same commit as the CHANGELOG entry
- [x] 4.7 Run the full test suite: `dotnet test --settings coverlet.runsettings` (all projects, all TFMs); confirm 95% coverage gate holds and no test is red
- [x] 4.8 Run `dotnet build` across all three TFMs and confirm clean build with zero warnings-as-errors
- [x] 4.9 Unit/integration test: lockstep `[Theory]` covers every non-success `ResultStatus` (happy path byte-identity) + 1 error case (Successful is excluded from the theory and tested to throw if called directly on the factory)
- [x] 4.10 e2e: `IceCreamTruck.WebApi.Tests` asserts JSON body shape through the real ASP.NET Core pipeline for both a controller action (Mvc mapper) and a Minimal API endpoint (Http mapper)
- [x] 4.11 Checkpoint: `dotnet test --settings coverlet.runsettings` passes in full; `dotnet build` is clean; `src/Directory.Build.props` reads `<Version>2.0.0-rc3</Version>`; `docs/errors/bad-request.md` (and the other four) exist on disk; `CHANGELOG.md` contains a `## [2.0.0-rc3]` breaking-change section
